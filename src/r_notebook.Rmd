...
```{r}
url="http://datacovid.org/api/answers/1/age;sortiesEn24h;confinementPieces;confinementNbPersonnes;diplome;confinementResidenceHabituelle;preoccupation1;penseInfecte;departement"
...
```{r}
france$comptedepartement=comptedepartement
france$comptedepartement[order(france$CC_2)]=comptedepartement
couleurs <- colorRampPalette(c('white', 'magenta'))
head(comptedepartement)
spplot(france, "comptedepartement", col.regions=couleurs(30),  main=list(label="Nombre de participants par département",cex=.8))
```
